...
export default async function ThankYouPageIt({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const page = await getThankYouPage(slug, "it");
  if (!page) notFound();

  const related = getAllResources("it")
